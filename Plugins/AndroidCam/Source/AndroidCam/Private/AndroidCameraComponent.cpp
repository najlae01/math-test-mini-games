...
void UAndroidCameraComponent::updateTexture(void* data)
{
	...
	//RegionData->Texture = androidCameraTexture;
	RegionData->Texture2DResource = (FTexture2DResource*)androidCameraTexture->GetResource();
		...
		[RegionData, bFreeData](FRHICommandListImmediate& RHICmdList)
		{
			for (uint32 RegionIndex = 0; RegionIndex < RegionData->NumRegions; ++RegionIndex)
			{
				if (RegionData->Texture2DResource)
				{
					...
					{
						// Access the texture RHI safely through GetResourceData
						FRHITexture2D* Texture2DRHI = RegionData->Texture2DResource->GetTextureRHI()->GetTexture2D();
