...
bool newFrame = false;
unsigned char* rawDataAndroid = nullptr;
#endif
...
void  AndroidThunkCpp_startCamera()
{
	...
	if (!AndroidThunkJava_startCamera || !ENV || !FJavaWrapper::GameActivityThis) {
		__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Error: JNI method or ENV is NULL");
		return;
	}
	...
	if (ENV->ExceptionCheck()) {
		ENV->ExceptionDescribe();  // Prints error details
		ENV->ExceptionClear();     // Clears the exception
		__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "JNI Exception occurred!");
	}
}
